...
export function calculateDiscountRisk(input: DiscountRiskInput): DiscountRiskResult {
  const { customerTier, requestedDiscountPct, allowedDiscountPct, marginPct, previousApprovalCount = 0 } = input;
  ...
  if (exceedsTierCeiling || marginPct < 10) {
    return {
      riskLevel: "HIGH",
      reason: exceedsTierCeiling
        ? `Discount exceeds ${customerTier} customer ceiling by ${(requestedDiscountPct - tierCeiling).toFixed(1)}%.`
  ...
  if (exceedsAllowed || requestedDiscountPct >= allowedDiscountPct * 0.9 || previousApprovalCount >= 2) {
    return {
      riskLevel: "MEDIUM",
      reason: exceedsAllowed
        ? `Discount exceeds the standard allowed limit by ${(requestedDiscountPct - allowedDiscountPct).toFixed(1)}%.`
